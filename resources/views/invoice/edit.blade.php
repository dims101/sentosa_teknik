        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-6">
                            {{ __('Edit Invoice ') }}{{$invoice->kode_invoice}}
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="/invoice/{{$invoice->id}}/update" onsubmit="cleanRupiahValues()">
                        @csrf
                        @method('put')
                        <div class="mb-3">
                            <label for="pelanggan" class="form-label">Pelanggan</label>
                            <input type="text" class="form-control" id="pelanggan" name="pelanggan" value="{{$invoice->pelanggan}}" placeholder="Masukan nama pelanggan" required>
                        </div>
                        <div class="mb-3">
                            <label for="telepon" class="form-label">Telepon</label>
                            <input type="number" class="form-control" id="telepon" name="telepon" value="{{$invoice->telepon}}" placeholder="Masukan nomor telepon">
                        </div>
                        <div class="mb-3">
                            <label for="pelanggan" class="form-label">Alamat</label>
                            <textarea name="alamat" id="alamat" rows="3" placeholder="Masukan alamat" class="form-control" required>{{$invoice->alamat}}</textarea>
                        </div>
                        <hr>
                        <div class="mb-3">
                            <label for="pengerjaan" class="form-label">Tanggal pengerjaan</label>
                            <input type="date" class="form-control" id="pengerjaan" value="{{$invoice->pengerjaan}}" name="pengerjaan" required>
                        </div>

                        <div class="mb-3">
                            <label for="teknisi" class="form-label">Teknisi</label>
                            <input type="text" class="form-control" id="teknisi" name="teknisi" value="{{$invoice->teknisi}}" placeholder="Masukan nama teknisi" required>
                        </div>
                        <div class="mb-3">
                            <label for="keterangan" class="form-label">Keterangan</label>
                            <textarea name="keterangan" id="keterangan" class="form-control" rows="3" placeholder="Tambah keterangan">{{$invoice->keterangan}}</textarea>
                        </div>
                        <hr>
                        ...
                        <div id="layananContainer">
                            ...
                            <div class="input-group mb-3">
                                <select name="layanan[]" class="form-control mr-3" onchange="hitungTotalHarga()" required>
                                    ...
                                    <option value="{{ $service->id }}" data-harga=" {{ $service->harga }}" {{$order->layanan_id == $service->id ? 'selected' : ''}}>{{ $service->layanan }} @Rp. {{ number_format($service->harga, 0, ',', '.') }}</option>
                                    @endforeach
                                </select>
                                <select name="jumlahLayanan[]" class="form-control mr-3" onchange="hitungTotalHarga()">
                                    @for($i=1;$i<=5;$i++) <option value="{{$i}}" {{$order->jumlah == $i ? 'selected' : ''}}>{{$i}}</option>
                                        @endfor
                                        <!-- Tambahkan opsi jumlah layanan sesuai kebutuhan -->
                                </select>
                                @if ($loop->iteration >1)
                                <button class="btn btn-sm btn-danger" onclick="hapusLayanan(this)">Hapus</button>
                                @endif
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="total_bayar" id="total_bayar" value="{{$invoice->total_bayar}}" class="form-control" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="dibayar">Telah dibayar</label>
                            <input type="text" name="dibayar" id="dibayar" class="form-control" value="{{$invoice->dibayar}}" oninput="hitungPelunasan();formatRupiah(this)" required>
                        </div>
                        <div class="mb-3">
                            <label for="pelunasan">Pelunasan</label>
                            <input type="text" name="pelunasan" id="pelunasan" value="{{$invoice->pelunasan}}" class="form-control" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="tempo" class="form-label">Tanggal Jatuh Tempo Pembayaran</label>
                            <input type="date" class="form-control" id="tempo" value="{{$invoice->tempo}}" name="tempo">
                        </div>
                        <hr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        dibayar.value = formatNumber(dibayar.value);
        pelunasan.value = formatNumber(pelunasan.value);
    });
